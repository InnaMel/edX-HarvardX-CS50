...
//    var newMarker = new MarkerWithLabel({
        ...
        /*title: placeName,*/
        label: placeName + ", " + place.admin_name1
    });
    ...
    newMarker.addListener('click', function() {
        ...
        $.getJSON("/articles", parameters, function(data, textStatus, jqXHR) {
            let myStringData = "<div><ul>";
            for (let i = 0; i < data.length; i++)
            {
                myStringData = myStringData + "<li><a href=\"" + data[i].link + "\">" + data[i].title + "</a></li>";
            }
            myStringData += "</ul></div>";
            //alert(myStringData);
//            infowindow.setContent("<div><li>new</b> data</p>"data
            infowindow.setContent(myStringData);
            infowindow.open(map, newMarker);
        });
