...
int main(int argc, char *argv[])
{
    ...
    outi.biSizeImage = abs(outi.biHeight) * (outi.biWidth * sizeof(RGBTRIPLE) + newPadding);

    outf.bfSize =  outi.biSizeImage + sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER);
    ...
    for (int i = 0, biHeight = abs(bi.biHeight); i < biHeight; i++)
    {
        ...
        size_t rezWriteOut = fwrite(newSLTriple, sizeof(RGBTRIPLE), outi.biWidth, outptr);
