...
int main(int argc, char* argv[])
{
    ...
    {   frd = fread(buff, 1, 512, recFile);
        ...
        {
            if(compare(oneJpgSTR) == false && found == true)
            {
                fwrite(oneJpgSTR, sizeJpgBlock, 1, img);
            }
            else
            if(compare(oneJpgSTR) == true && found == true)
            {
                numberJpg++;
                fclose(img);
                sprintf(filename, "%03i.jpg", numberJpg);
...
{
    bool rezCompare;
    if(oneJpgSTR->SOI[0] == 0xff && oneJpgSTR->SOI[1] == 0xd8 && oneJpgSTR->APP0[0] == 0xff && (oneJpgSTR->APP0[1] & 0xf0) == 0xe0)
    {
        rezCompare = true;
    }
    else
